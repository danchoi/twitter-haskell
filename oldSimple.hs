...
authenticate consumer = runClient . runOAuth $ do
...
singleAccessToken consumer accToken accSecret = runClient . runOAuth $ do
...
tweet consumer token message = runClient . runOAuth $ do
    ignite $ Application (key consumer) (secret consumer) OOB
    putToken token
